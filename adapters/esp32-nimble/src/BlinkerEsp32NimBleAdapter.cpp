...
Esp32NimBleLink::Esp32NimBleLink(const Esp32NimBleLinkConfig& config)
    : config_(config),
...
void Esp32NimBleLink::poll(uint32_t) {
    if (state_ != BleLinkState::Ready) return;
    processPendingEvents();
    drainPackets();
}

BleLinkState Esp32NimBleLink::state() const { return state_; }
ErrorCode Esp32NimBleLink::lastError() const { return lastError_; }
size_t Esp32NimBleLink::sessionCount() const {
    return sessionAnnounced_ ? 1U : 0U;
}

Result Esp32NimBleLink::sessionAt(
    size_t index,
...
Result Esp32NimBleLink::sendPacket(
    uint32_t sessionId,
    ByteView packet) {
...
void Esp32NimBleLink::setPacketReceiver(
...
void Esp32NimBleLink::setSessionHandlers(
...
Result Esp32NimBleLink::configureBleProfile(
...
Result Esp32NimBleLink::setSessionSecurity(
    uint32_t sessionId,
    bool encrypted,
    bool bonded,
    bool authenticated) {
...
void Esp32NimBleLink::onConnect(
    NimBLEServer* server,
...
void Esp32NimBleLink::onDisconnect(
    NimBLEServer*,
    NimBLEConnInfo& connection,
    int) {
    const uint16_t handle = connection.getConnHandle();
...
void Esp32NimBleLink::onMTUChange(
    uint16_t mtu,
    NimBLEConnInfo& connection) {
    const uint16_t handle = connection.getConnHandle();
    portENTER_CRITICAL(&lock_);
...
void Esp32NimBleLink::onAuthenticationComplete(
    NimBLEConnInfo& connection) {
    portENTER_CRITICAL(&lock_);
...
void Esp32NimBleLink::onSubscribe(
    NimBLECharacteristic* characteristic,
    NimBLEConnInfo& connection,
...
void Esp32NimBleLink::processPendingEvents() {
    bool connect = false;
...
void Esp32NimBleLink::drainPackets() {
    uint8_t delivered = 0;
...
void Esp32NimBleLink::clearPacketQueueLocked() {
    packetHead_ = 0;
    packetTail_ = 0;
    packetCount_ = 0;
}

uint32_t Esp32NimBleLink::nextSessionId() {
...
uint16_t Esp32NimBleLink::packetSizeForMtu(uint16_t mtu) {
    if (mtu <= 3U) return 0U;
...
bool Esp32NimBleLink::validConfig() const {
