...
ArduinoBleLink* ArduinoBleLink::active_ = nullptr;

ArduinoBleLink::ArduinoBleLink(const ArduinoBleLinkConfig& config)
    : ArduinoBleLink(nullptr, config) {}

ArduinoBleLink::ArduinoBleLink(
...
void ArduinoBleLink::poll(uint32_t) {
...
BleLinkState ArduinoBleLink::state() const { return state_; }
ErrorCode ArduinoBleLink::lastError() const { return lastError_; }
size_t ArduinoBleLink::sessionCount() const {
    return sessionAnnounced_ ? 1U : 0U;
}

Result ArduinoBleLink::sessionAt(
    size_t index,
...
Result ArduinoBleLink::sendPacket(
    uint32_t sessionId,
    ByteView packet) {
...
void ArduinoBleLink::setPacketReceiver(
...
void ArduinoBleLink::setSessionHandlers(
...
Result ArduinoBleLink::configureBleProfile(
...
void ArduinoBleLink::updateConnection() {
...
void ArduinoBleLink::connectedThunk(BLEDevice device) {
    if (active_ != nullptr) active_->onConnected(device);
}

void ArduinoBleLink::disconnectedThunk(BLEDevice device) {
    if (active_ != nullptr) active_->onDisconnected(device);
}

void ArduinoBleLink::onConnected(BLEDevice device) {
    if (!device) return;
    ...
    central_ = device;
    if (security_ != nullptr) security_->connected(device);
    connectPending_ = true;
}

void ArduinoBleLink::onDisconnected(BLEDevice device) {
...
uint32_t ArduinoBleLink::nextSessionId() {
...
bool ArduinoBleLink::validConfig() const {
