#ifndef BLINKER_V2_ARDUINO_ESP32_WIFI_PROV_PRODUCT_H
#define BLINKER_V2_ARDUINO_ESP32_WIFI_PROV_PRODUCT_H
...
enum class Esp32WifiProvProductState : uint8_t {
    Stopped = 0U,
    Provisioning,
    Cloud,
    Fault
...
class Esp32WifiProvProduct final
    : public IProductLifecycle {
public:
    Esp32WifiProvProduct()
        : platform_(),
          ...
          serviceName_(),
          proofOfPossession_(),
          state_(Esp32WifiProvProductState::Stopped),
          lastError_(ErrorCode::Ok),
          configured_(false),
          forceProvisioning_(false) {}

    ~Esp32WifiProvProduct() override {
        stop();
        secureZero(MutableByteSpan(
    ...
    Result configure(
        ...
        if (state_ != Esp32WifiProvProductState::Stopped || configured_) {
    ...
    Result start() override {
        if (state_ != Esp32WifiProvProductState::Stopped) {
            return Result::failure(ErrorCode::AlreadyExists);
        }
        ...
        state_ = Esp32WifiProvProductState::Provisioning;
        lastError_ = ErrorCode::Ok;
        return Result::success();
    }

    void poll(uint32_t totalBudgetMicros) override {
        if (state_ == Esp32WifiProvProductState::Provisioning) {
            provisioner_.poll();
        ...
        } else if (state_ == Esp32WifiProvProductState::Cloud) {
    ...
    ProductLifecycleStatus status() const override {
        ...
        if (state_ == Esp32WifiProvProductState::Fault) {
            return ProductLifecycleStatus(
...
private:
    ...
    Result startCloud() {
        ...
        if (!result) return failStart(result.code());
        state_ = Esp32WifiProvProductState::Cloud;
        lastError_ = ErrorCode::Ok;
        return Result::success();
    }
    ...
    void enterFault(ErrorCode error) {
        ...
        lastError_ = error == ErrorCode::Ok
                         ...
                         : error;
        state_ = Esp32WifiProvProductState::Fault;
    }
    ...
    Esp32WifiProvProductState state_;
    ErrorCode lastError_;
    bool configured_;
    bool forceProvisioning_;

    Esp32WifiProvProduct(const Esp32WifiProvProduct&);
    Esp32WifiProvProduct& operator=(const Esp32WifiProvProduct&);
};
