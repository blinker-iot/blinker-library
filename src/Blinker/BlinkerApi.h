...
class BlinkerApi : public BlinkerProtocol
{
    ...
    protected :
        ...
        uint32_t debug_time = 0;
            ...
            String getIMEI(uint32_t time_out = 1000);
            bool httpGET(const String & host, const String & uri, uint32_t time_out = 5000);
            bool ttpPOST(const String & _host, const String & _uri, 
    ...
    switch (BProto::state)
    {
        ...
        case CONNECTED :
            if (conState)
            {
                ...
                #if (defined(BLINKER_MQTT) || defined(BLINKER_PRO) || defined(BLINKER_GATEWAY))
                    #if defined(BLINKER_ALIGENIE)
...
char * BlinkerApi::widgetName_int(uint8_t num)
{
...
#if defined(BLINKER_MQTT) || defined(BLINKER_PRO) || \
    defined(BLINKER_GATEWAY)// || defined(BLINKER_MQTT_AT)
    ...
    String BlinkerApi::getIMEI(uint32_t time_out)
    {
        uint32_t http_time = millis();

        BProto::serialPrint(BLINEKR_CMD_CGSN_RESQ);
