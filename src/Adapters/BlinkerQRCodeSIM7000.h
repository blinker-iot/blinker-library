...
#endif
#include "../Functions/BlinkerMQTTSIM7000.h"
...
BlinkerMQTTSIM7000* mqtt_NBIoT;

class BlinkerQRCodeSIM7000 : public BlinkerStream
{
    public :
        BlinkerQRCodeSIM7000()
        ...
        void begin(const char* _auth, const char* _type, String _imei);
...
int BlinkerQRCodeSIM7000::connect()
{
...
int BlinkerQRCodeSIM7000::connected()
{
    if (!isMQTTinit) return false;
...
int BlinkerQRCodeSIM7000::mConnected()
{
    if (!isMQTTinit) return false;
    else return mqtt_NBIoT->connected();
}

void BlinkerQRCodeSIM7000::disconnect()
{
    if (isMQTTinit) {
        isConnect = false;
...
void BlinkerQRCodeSIM7000::ping()
{
    ...
    if (BLINKER_SIM7000.isReboot())
    {
        BLINKER_SIM7000.powerCheck();

        // disconnect();
    }
}

int BlinkerQRCodeSIM7000::available()
{
    if (isMQTTinit) {
        checkKA();
...
int BlinkerQRCodeSIM7000::aligenieAvail()
{
    if (!isMQTTinit) return false;
...
int BlinkerQRCodeSIM7000::duerAvail()
{
    if (!isMQTTinit) return false;
...
void BlinkerQRCodeSIM7000::subscribe()
{
    if (!isMQTTinit) return;
...
void BlinkerQRCodeSIM7000::flush()
{
    if (isFresh_NBIoT)
    {
...
int BlinkerQRCodeSIM7000::print(char * data, bool needCheck)
{
    if (!isMQTTinit) return false;
...
int BlinkerQRCodeSIM7000::toServer(char * data)
{
    if (!isMQTTinit) return false;    
...
int BlinkerQRCodeSIM7000::bPrint(char * name, const String & data)
{
    if (!isMQTTinit) return false;
...
int BlinkerQRCodeSIM7000::aliPrint(const String & data)
{
    if (!isMQTTinit) return false;
...
int BlinkerQRCodeSIM7000::duerPrint(const String & data, bool report)
{
    if (!isMQTTinit) return false;
...
void BlinkerQRCodeSIM7000::aliType(const String & type)
{
...
void BlinkerQRCodeSIM7000::duerType(const String & type)
{
...
void BlinkerQRCodeSIM7000::begin(const char* _auth, const char* _type, String _imei)
{
    _authKey = _auth;
    _deviceType = _type;
...
void BlinkerQRCodeSIM7000::initStream(Stream& s, bool state, blinker_callback_t func)
{
    // _deviceType = _type;
...
int BlinkerQRCodeSIM7000::autoPrint(unsigned long id)
{
...
char * BlinkerQRCodeSIM7000::deviceName()
{
    BLINKER_LOG_ALL(BLINKER_F("BlinkerQRCodeSIM7000::deviceName: "), MQTT_ID_NBIoT);
    return MQTT_ID_NBIoT;/*MQTT_ID_PRO;*/
}

void BlinkerQRCodeSIM7000::sharers(const String & data)
{
...
int BlinkerQRCodeSIM7000::connectServer()
{
    ...
    // uri += imei;
    uri += BLINKER_F("/api/v1/user/device/scancode/add");
    // uri += BLINKER_F("/post");
    // uri += _deviceType;
    // uri += _aliType;
    // uri += _duerType;
    ...
    String post_data = "{\"deviceType\":\"";
    post_data += STRING_format(_deviceType);
    post_data += "\",\"typeKey\":\"";
    post_data += STRING_format(_authKey);
    post_data += "\",\"registerKey\":\"";
    post_data += imei;
    post_data += "\",\"expiresIn\": 60}";

    String payload;

    if (http.POST(post_data, "", ""))
    {
        BLINKER_LOG(BLINKER_F("[HTTP] POST... success"));
    ...
    {
        BLINKER_LOG(BLINKER_F("[HTTP] POST... failed"));
    ...
    // if (_broker == BLINKER_MQTT_BORKER_ALIYUN) {
        mqtt_NBIoT = new BlinkerMQTTSIM7000(*stream, isHWS, MQTT_HOST_NBIoT, MQTT_PORT_NBIoT, 
...
void BlinkerQRCodeSIM7000::checkKA()
{
    if (millis() - kaTime >= BLINKER_MQTT_KEEPALIVE)
        isAlive = false;
}

int BlinkerQRCodeSIM7000::checkAliKA() {
...
int BlinkerQRCodeSIM7000::checkDuerKA() {
...
int BlinkerQRCodeSIM7000::checkCanPrint() {
...
int BlinkerQRCodeSIM7000::checkCanBprint() {
...
int BlinkerQRCodeSIM7000::checkPrintSpan()
{
...
int BlinkerQRCodeSIM7000::checkAliPrintSpan()
{
...
int BlinkerQRCodeSIM7000::checkDuerPrintSpan()
{
...
int BlinkerQRCodeSIM7000::isJson(const String & data)
{
