#ifndef BLINKER_SERIAL_SIM7000_H
#define BLINKER_SERIAL_SIM7000_H
...
class BlinkerSerialSIM7000 : public BlinkerStream
{
    public :
        BlinkerSerialSIM7000()
...
int BlinkerSerialSIM7000::connected()
{
    if (!isMQTTinit) return false;
...
int BlinkerSerialSIM7000::mConnected()
{
    if (!isMQTTinit) return false;
    else return mqtt_NBIoT->connected();
}

void BlinkerSerialSIM7000::disconnect()
{
    if (isMQTTinit) {
        isConnect = false;
        mqtt_NBIoT->disconnect();
...
void BlinkerSerialSIM7000::ping()
{
...
int BlinkerSerialSIM7000::available()
{
    if (isMQTTinit) {
        checkKA();
...
int BlinkerSerialSIM7000::aligenieAvail()
{
    if (!isMQTTinit) return false;
...
int BlinkerSerialSIM7000::duerAvail()
{
    if (!isMQTTinit) return false;
...
void BlinkerSerialSIM7000::subscribe()
{
    if (!isMQTTinit) return;
...
void BlinkerSerialSIM7000::flush()
{
    if (isFresh_NBIoT)
    {
...
int BlinkerSerialSIM7000::print(char * data, bool needCheck)
{
    if (!isMQTTinit) return false;
...
int BlinkerSerialSIM7000::toServer(char * data)
{
    if (!isMQTTinit) return false;    
...
int BlinkerSerialSIM7000::bPrint(char * name, const String & data)
{
    if (!isMQTTinit) return false;
...
int BlinkerSerialSIM7000::aliPrint(const String & data)
{
    if (!isMQTTinit) return false;
...
int BlinkerSerialSIM7000::duerPrint(const String & data, bool report)
{
    if (!isMQTTinit) return false;
...
void BlinkerSerialSIM7000::aliType(const String & type)
{
...
void BlinkerSerialSIM7000::duerType(const String & type)
{
...
void BlinkerSerialSIM7000::begin(const char* _type, String _imei)
{
    _deviceType = _type;
...
void BlinkerSerialSIM7000::initStream(Stream& s, bool state, blinker_callback_t func)
{
    // _deviceType = _type;
...
int BlinkerSerialSIM7000::autoPrint(unsigned long id)
{
...
char * BlinkerSerialSIM7000::deviceName()
{
    BLINKER_LOG_ALL(BLINKER_F("BlinkerSerialSIM7000::deviceName: "), MQTT_ID_NBIoT);
    return MQTT_ID_NBIoT;/*MQTT_ID_PRO;*/
}

void BlinkerSerialSIM7000::sharers(const String & data)
{
...
int BlinkerSerialSIM7000::connectServer()
{
...
void BlinkerSerialSIM7000::checkKA()
{
    if (millis() - kaTime >= BLINKER_MQTT_KEEPALIVE)
        isAlive = false;
}

int BlinkerSerialSIM7000::checkAliKA() {
...
int BlinkerSerialSIM7000::checkDuerKA() {
...
int BlinkerSerialSIM7000::checkCanPrint() {
...
int BlinkerSerialSIM7000::checkCanBprint() {
...
int BlinkerSerialSIM7000::checkPrintSpan()
{
...
int BlinkerSerialSIM7000::checkAliPrintSpan()
{
...
int BlinkerSerialSIM7000::checkDuerPrintSpan()
{
...
int BlinkerSerialSIM7000::isJson(const String & data)
{
