#ifndef BLINKER_PRO_SIM7020_H
#define BLINKER_PRO_SIM7020_H
...
class BlinkerProSIM7020 : public BlinkerStream
{
    public :
        BlinkerProSIM7020()
...
int BlinkerProSIM7020::connected()
{
    if (!isMQTTinit) return false;
...
int BlinkerProSIM7020::mConnected()
{
    if (!isMQTTinit) return false;
    else return mqtt_NBIoT->connected();
}

void BlinkerProSIM7020::disconnect()
{
    if (isMQTTinit) mqtt_NBIoT->disconnect();
}

void BlinkerProSIM7020::ping()
{
...
int BlinkerProSIM7020::available()
{
    if (isMQTTinit) {
        checkKA();
...
int BlinkerProSIM7020::aligenieAvail()
{
    if (!isMQTTinit) return false;
...
int BlinkerProSIM7020::duerAvail()
{
    if (!isMQTTinit) return false;
...
void BlinkerProSIM7020::subscribe()
{
    if (!isMQTTinit) return;
...
int BlinkerProSIM7020::print(char * data, bool needCheck)
{
    if (!isMQTTinit) return false;
    ...
    strcat(data, MQTT_DEVICEID_NBIoT);
...
int BlinkerProSIM7020::bPrint(char * name, const String & data)
{
    ...
    strcat(data_add, MQTT_DEVICEID_NBIoT);
...
int BlinkerProSIM7020::aliPrint(const String & data)
{
    if (!isMQTTinit) return false;
...
int BlinkerProSIM7020::duerPrint(const String & data, bool report)
{
    if (!isMQTTinit) return false;
...
// void BlinkerProSIM7020::aliType(const String & type)
// {
...
// void BlinkerProSIM7020::duerType(const String & type)
// {
...
void BlinkerProSIM7020::begin(const char* _type, String _imei)
{
    _deviceType = _type;
...
void BlinkerProSIM7020::initStream(Stream& s, bool state, blinker_callback_t func)
{
    // _deviceType = _type;
...
int BlinkerProSIM7020::autoPrint(unsigned long id)
{
    ...
    payload += STRING_format(MQTT_DEVICEID_NBIoT);
...
char * BlinkerProSIM7020::deviceName()
{
    BLINKER_LOG_ALL(BLINKER_F("BlinkerProSIM7020::deviceName: "), MQTT_DEVICEID_NBIoT);
    return MQTT_DEVICEID_NBIoT;/*MQTT_ID_PRO;*/
}

void BlinkerProSIM7020::sharers(const String & data)
{
...
int BlinkerProSIM7020::authCheck()
{
    uint8_t _authCheck;
...
int BlinkerProSIM7020::connectServer()
{
    ...
    if (_broker == BLINKER_MQTT_BORKER_ALIYUN) {
        ...
        MQTT_DEVICEID_NBIoT = (char*)malloc((_userID.length()+1)*sizeof(char));
        strcpy(MQTT_DEVICEID_NBIoT, _userID.c_str());
        ...
        AUTHKEY_NBIoT = (char*)malloc((_authKey.length()+1)*sizeof(char));
        strcpy(AUTHKEY_NBIoT, _authKey.c_str());
    ...
    BLINKER_LOG_ALL(BLINKER_F("===================="));
    BLINKER_LOG_ALL(BLINKER_F("DEVICE_NAME_NBIoT: "), imei);
    BLINKER_LOG_ALL(BLINKER_F("MQTT_PRODUCTINFO_NBIoT: "), MQTT_PRODUCTINFO_NBIoT);
    BLINKER_LOG_ALL(BLINKER_F("MQTT_DEVICEID_NBIoT: "), MQTT_DEVICEID_NBIoT);
    ...
    BLINKER_LOG_ALL(BLINKER_F("AUTHKEY_NBIoT: "), AUTHKEY_NBIoT);
    BLINKER_LOG_ALL(BLINKER_F("===================="));
    ...
    if (_broker == BLINKER_MQTT_BORKER_ALIYUN) {
        ...
        PUB_TOPIC_STR += MQTT_DEVICEID_NBIoT;
        ...
        SUB_TOPIC_STR += MQTT_DEVICEID_NBIoT;
...
void BlinkerProSIM7020::checkKA()
{
    if (millis() - kaTime >= BLINKER_MQTT_KEEPALIVE)
        isAlive = false;
}

int BlinkerProSIM7020::checkAliKA() {
...
int BlinkerProSIM7020::checkDuerKA() {
...
int BlinkerProSIM7020::checkCanPrint() {
...
int BlinkerProSIM7020::checkCanBprint() {
...
int BlinkerProSIM7020::checkPrintSpan()
{
...
int BlinkerProSIM7020::checkAliPrintSpan()
{
...
int BlinkerProSIM7020::checkDuerPrintSpan()
{
...
int BlinkerProSIM7020::isJson(const String & data)
{
