#ifndef BLINKER_BLE_SERIAL_H
#define BLINKER_BLE_SERIAL_H
...
BlinkerBLESerial    BLESerial;

int BlinkerBLESerial::available()
{
    if (!isHWS)
    {
...
void BlinkerBLESerial::begin(Stream& s, bool state)
{
    stream = &s;
...
int BlinkerBLESerial::timedRead()
{
    int c;
...
// int BlinkerBLESerial::print(const String & s, bool needCheck)
int BlinkerBLESerial::print(char * data, bool needCheck)
{
    if (needCheck)
    {
        if (!checkPrintSpan())
...
int BlinkerBLESerial::checkPrintSpan()
{
